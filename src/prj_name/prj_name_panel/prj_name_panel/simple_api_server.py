...
from prj_name_panel.dispatcher_client import DispatcherClient
from prj_name_panel.rmf_msg_observer import AsyncRmfMsgObserver
from prj_name_panel.rmf_msg_observer import RmfMsgType
...
# Note: Get Dashboard Config for each "World", specific to prj_name impl
...
def main(args=None):
    ...
    if 'SIMPLE_API_SERVER_IP' in os.environ:
        server_ip = os.environ['SIMPLE_API_SERVER_IP']
        print(f'Set Server IP to: {server_ip}')

    if 'SIMPLE_API_SERVER_PORT' in os.environ:
        port_num = int(os.environ['SIMPLE_API_SERVER_PORT'])
    ...
    print(
        f'Starting Simple API Server: {server_ip}:{port_num}, '
