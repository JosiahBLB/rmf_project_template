...
from fyp_panel.dispatcher_client import DispatcherClient
from fyp_panel.rmf_msg_observer import AsyncRmfMsgObserver
from fyp_panel.rmf_msg_observer import RmfMsgType
...
# Note: Get Dashboard Config for each "World", specific to fyp impl
...
def main(args=None):
    ...
    if 'FYP_API_SERVER_IP' in os.environ:
        server_ip = os.environ['FYP_API_SERVER_IP']
        print(f'Set Server IP to: {server_ip}')

    if 'FYP_API_SERVER_PORT' in os.environ:
        port_num = int(os.environ['FYP_API_SERVER_PORT'])
    ...
    print(
        f'Starting FYP API Server: {server_ip}:{port_num}, '
